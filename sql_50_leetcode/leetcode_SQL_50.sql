...
select to_char(trans_date, 'YYYY-MM') as month, country, count(state) trans_count, count(case when state = 'approved' then state end) as approved_count, sum (amount) as trans_total_amount ,coalesce(sum(case when state = 'approved' then amount end),0) as approved_total_amount
from Transactions
group by month, country

select to_char(trans_date,'yyyy-mm') as month,country, count(*) trans_count, 
...
SELECT 
    p.product_id, 
    ROUND(COALESCE(SUM(u.units * p.price) / SUM(units)::numeric, 0), 2) AS average_price
FROM 
    Prices p LEFT JOIN 
    UnitsSold u ON 
...
*/
-- Write your PostgreSQL query statement below
select query_name,round(avg((rating::decimal/position)),2) as quality, 
round(avg(case when (rating < 3) then 1
     else 0
end)*100,2)

as poor_query_percentage from Queries group by query_name having query_name is not null
